...
2. **Capabilities stay default-deny.** Same as today: `system` needs the server
   flag (`runtime.allow_system_agents`) AND the agent capability; `network` is
...
`system` code node only executes when `runtime.allow_system_agents` + the agent
...
- **Not a global grant.** `runtime.allow_system_agents` is a *ceiling* the
