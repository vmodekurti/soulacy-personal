...
|---|---|---|
| **P1 Model store** | `internal/person`: SQLite store with the schema below; owner-scoped like memory; every entry carries `source`, `observed_at`, `confidence`, `expires_at`. `GET/PUT /person/model`, `GET /person/model/sync?since=` (same change-feed shape as memory). | Phone caches the model in the app group (like `MemoryCache`); a "About you" screen in Settings shows it, lets the person correct or delete any entry, and export it. Web: a Person page. |
| **P2 One tool** | Builtin `person.model` for agents: `person.model(section, query?)` returns the relevant part in plain prose plus structured rows; `person.observe(entry)` lets an agent record something it learned, marked `source: agent:<id>`. Risk tier: read is safe, observe is write. | Agent templates gain `person.model` in their builtins. |
| **P3 Observers** | Small scheduled digesters, not LLM agents where a rule will do: **routine** (from `location.current`, `motion.current`, `focus.status` samples: usual wake, leave, arrive, sleep windows and the deviation of today), **commitments** (from calendar, reminders, email threads with a question or a deadline), **relationships** (from contacts, calendar attendees, message senders: who, how often, what is pending), **state** (from health summary, Focus, time of day, travel). Each writes only its section and never overwrites a manual entry. | Phone answers the sampling commands in the background where iOS allows (location, Focus, health summary once a day). |
| **P4 Consent and visibility** | Each observer is a capability switch with a plain-language purpose string, off by default, on per sense. Audit log entry per write. | Onboarding step "What may Soulacy notice?" with per-sense switches; the same switches in Settings. |
