    ...
    <div class="grid">
      <section class="card wide"><div class="section-title"><div><h2>LLM</h2><p>Choose task-specific provider and model roles just as in Personal mode. Configure credentials and discover models on <a href="#providers">Providers &amp; models</a>.</p></div><span class="scope">WORKSPACE</span></div>
        ...
        <div class="model-grid">
          ...
          <div class="model"><h3>Chat fallback</h3><p>Used only when the selected agent has no provider/model assignment.</p><ModelFields bind:provider={chatProvider} bind:model={chatModel} {providers} models={modelsByProvider[chatProvider] || []} loading={!!modelsLoading[chatProvider]} error={modelsError[chatProvider] || ''} {writable} optional on:providerchange={(event) => loadModels(event.detail.provider)} on:refresh={(event) => loadModels(event.detail.provider, true)} /></div>
          ...
          <div class="model"><h3>Reasoner fallback</h3><p>Used only when an agent has no provider/model assignment.</p><ModelFields bind:provider={reasonerProvider} bind:model={reasonerModel} {providers} models={modelsByProvider[reasonerProvider] || []} loading={!!modelsLoading[reasonerProvider]} error={modelsError[reasonerProvider] || ''} {writable} optional on:providerchange={(event) => loadModels(event.detail.provider)} on:refresh={(event) => loadModels(event.detail.provider, true)} /></div>
        </div>
      </section>
