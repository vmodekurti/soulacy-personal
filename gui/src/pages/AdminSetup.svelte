<script>
  ...
  async function restartAndOpenAdmin() {
...
<div class="setup-shell"><PublicNav current="admin" helpPage="admin-setup" /><main class="setup-main">
  <section class="setup-card">
    <div class="brand">⬡ <span>Soulacy</span></div>
    <p class="eyebrow">TEAM &amp; SCALE</p>
    <h1>First-time deployment setup</h1>
    <p class="lead">Bootstrap a new deployment with the host setup key. Returning deployment administrators should use the control plane instead.</p>
    <a class="control-plane-link" href="/admin">Open deployment admin login →</a>
    ...
    {:else if connected && required && !complete}
      ...
      <div class="grid two">
        ...
        <label>Workspace address <span>(optional)</span><input bind:value={workspaceSlug} pattern="[a-z0-9][a-z0-9-]{1,46}[a-z0-9]" placeholder="acme-primary" /></label>
    ...
    {:else if complete}
      ...
      <button class="primary" disabled={restarting} on:click={restartAndOpenAdmin}>{restarting ? 'Restarting gateway…' : 'Restart and open control plane'}</button>
    {/if}
    {/if}
