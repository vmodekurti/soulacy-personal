<script>
  ...
  let config=null, loading=true, error='', saving=false, complete=false, publicDemo=false
  ...
  onMount(load)
  async function load(){loading=true;error='';try{const r=await fetch(`/api/v1/auth/workspaces/${encodeURIComponent(workspaceID)}/config`);const j=await r.json().catch(()=>({}));if(!r.ok){config=j.workspace||null;const fallback=r.status===404?'Workspace was not found.':r.status>=500?'Soulacy is temporarily unavailable. Please try again shortly.':'Workspace login could not be loaded.';throw new Error(j.error||fallback)}config=j.workspace;publicDemo=!!j.public_demo;canonicalWorkspaceID=config.workspace_id;try{const saved=JSON.parse(localStorage.getItem('soulacy.recentWorkspaces')||'[]');const item={id:config.workspace_id,slug:config.workspace_slug||config.workspace_id,name:config.workspace_name,organization:config.organization_name,logo:config.workspace_logo||config.organization_logo||''};localStorage.setItem('soulacy.recentWorkspaces',JSON.stringify([item,...saved.filter((entry)=>entry.id!==item.id)].slice(0,4)))}catch(_){}}catch(e){error=e?.message||'Soulacy is temporarily unavailable. Please try again shortly.'}finally{loading=false}}
...
<div class="access-shell"><PublicNav current="workspace" helpPage="workspace-login" /><main><section class="card">
  ...
  {:else if config}
    ...
    {#if config.identity_status === 'active'}
      <p class="lead">{invitationToken?'Sign in with the invited email address to accept your workspace invitation.':publicDemo?'Try Soulacy with Google—no invitation required. You can create and test private, temporary Studio drafts; publishing and workspace administration stay disabled.':'Sign in using this workspace’s identity provider.'}</p>
      {#if publicDemo}<div class="demo-note"><strong>Safe public demo</strong><span>Your demo access and saved drafts expire automatically. Usage, models, and tools are capped to protect the shared workspace.</span></div>{/if}
      <button class="primary signin" on:click={signIn}><ProviderIcon provider={config.provider_type}/><span>{publicDemo?'Start the demo with ':'Continue with '}{providers.find(p=>p[0]===config.provider_type)?.[1]||'your organization'}</span></button>
    {:else if token}
