...
func TestTeamsSendPostsTextPayload(t *testing.T) {
	...
	a, err := New("teams", srv.URL, "Daily Report", time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTeamsSendNon2xxReturnsError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "bad hook", http.StatusBadGateway)
	}))
	defer srv.Close()
	a, err := New("teams", srv.URL, "", time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTeamsRegistryRequiresWebhookURL(t *testing.T) {
	_, ok, err := registry.NewChannel("teams", map[string]any{})
	if !ok {
		t.Fatal("teams factory not registered")
	}
