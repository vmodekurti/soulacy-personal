...
type LLMConfig struct {
	...
	// Reasoner optionally supplies a provider/model fallback for multi-step
	// reasoning loops (ReAct / Plan-Execute). An agent's own llm.provider/model
	// assignment always wins; this pair is used only when both agent fields are
	// empty. Only providers with a reasoning backend are honored.
	//
	//	llm:
	//	  reasoner:
	//	    provider: anthropic
...
func Load(cfgPath string) (*Config, string, error) {
	...
	v.SetDefault("storage.command", "")
	v.SetDefault("storage.args", []string{})
	v.SetDefault("vector.backend", "") // empty → inherit from memory.vector_db
	v.SetDefault("vector.url", "")
	v.SetDefault("vector.collection", "soulacy_memory")
	v.SetDefault("vector.api_key", "")
	v.SetDefault("vector.dims", 768)
	v.SetDefault("vector.command", "")
	v.SetDefault("vector.args", []string{})
