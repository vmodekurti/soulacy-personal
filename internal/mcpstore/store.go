...
// to review and install third-party MCP code. It deliberately contains no
// approval token: approval always performs a fresh repository inspection.
type InstallRequest struct {
	...
	InstalledServerID string     `json:"installed_server_id,omitempty"`
}
...
CREATE TABLE IF NOT EXISTS workspace_mcp_install_requests(
	workspace_id       TEXT NOT NULL,
	id                 TEXT NOT NULL,
	...
	installed_server_id TEXT NOT NULL DEFAULT '',
	PRIMARY KEY (workspace_id, id)
);
CREATE INDEX IF NOT EXISTS idx_workspace_mcp_requests_status
	ON workspace_mcp_install_requests(workspace_id, status, requested_at DESC);`
...
// CreateInstallRequest records one developer request. IDs are generated by
// the gateway and are unique within the workspace.
func (s *Store) CreateInstallRequest(ctx context.Context, request InstallRequest) error {
	if s == nil || s.db == nil {
		return ErrUnavailable
	}
	request.WorkspaceID = wsroot.Normalize(request.WorkspaceID)
	request.ID = strings.TrimSpace(request.ID)
	request.SourceURL = strings.TrimSpace(request.SourceURL)
	request.RequestedBy = strings.TrimSpace(request.RequestedBy)
	if request.ID == "" || request.SourceURL == "" || request.RequestedBy == "" {
		return fmt.Errorf("mcpstore: request id, source URL, and requester are required")
	}
	if request.Status == "" {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO workspace_mcp_install_requests(
		workspace_id,id,source_url,reason,network,workspace_access,status,requested_by,requested_at)
		VALUES(?,?,?,?,?,?,?,?,?)`, request.WorkspaceID, request.ID, request.SourceURL,
		request.Reason, request.Network, request.WorkspaceAccess, request.Status,
		request.RequestedBy, request.RequestedAt)
	return err
}

// GetInstallRequest returns one request from one workspace. Cross-workspace
// lookups intentionally have the same result as an unknown ID.
func (s *Store) GetInstallRequest(ctx context.Context, workspaceID, id string) (InstallRequest, error) {
	if s == nil || s.db == nil {
		return InstallRequest{}, ErrUnavailable
	}
	request := InstallRequest{WorkspaceID: wsroot.Normalize(workspaceID)}
	var decidedAt sql.NullTime
	err := s.db.QueryRowContext(ctx, `SELECT id,source_url,reason,network,workspace_access,status,
		requested_by,requested_at,decided_by,decided_at,decision_reason,installed_server_id
		FROM workspace_mcp_install_requests WHERE workspace_id=? AND id=?`,
		request.WorkspaceID, strings.TrimSpace(id)).Scan(&request.ID, &request.SourceURL,
		&request.Reason, &request.Network, &request.WorkspaceAccess, &request.Status,
		&request.RequestedBy, &request.RequestedAt, &request.DecidedBy, &decidedAt,
		&request.DecisionReason, &request.InstalledServerID)
	if errors.Is(err, sql.ErrNoRows) {
		return InstallRequest{}, sql.ErrNoRows
	}
	if err != nil {
...
// ListInstallRequests returns a workspace's requests. requester is optional;
// handlers use it to limit developers to their own submissions.
func (s *Store) ListInstallRequests(ctx context.Context, workspaceID, requester string) ([]InstallRequest, error) {
	if s == nil || s.db == nil {
		return nil, ErrUnavailable
	}
	query := `SELECT id,source_url,reason,network,workspace_access,status,requested_by,
		requested_at,decided_by,decided_at,decision_reason,installed_server_id
		FROM workspace_mcp_install_requests WHERE workspace_id=?`
	args := []any{wsroot.Normalize(workspaceID)}
	if strings.TrimSpace(requester) != "" {
		query += ` AND requested_by=?`
		args = append(args, strings.TrimSpace(requester))
	}
	...
	for rows.Next() {
		request := InstallRequest{WorkspaceID: wsroot.Normalize(workspaceID)}
		var decidedAt sql.NullTime
		if err := rows.Scan(&request.ID, &request.SourceURL, &request.Reason, &request.Network,
			&request.WorkspaceAccess, &request.Status, &request.RequestedBy, &request.RequestedAt,
			&request.DecidedBy, &decidedAt, &request.DecisionReason, &request.InstalledServerID); err != nil {
			return nil, err
		}
		if decidedAt.Valid {
			value := decidedAt.Time
...
// DecideInstallRequest closes a pending request exactly once.
func (s *Store) DecideInstallRequest(ctx context.Context, workspaceID, id, status, actor, reason, serverID string) error {
	if s == nil || s.db == nil {
		return ErrUnavailable
	}
	if status != RequestInstalled && status != RequestDenied {
		return fmt.Errorf("mcpstore: invalid request decision %q", status)
	}
	result, err := s.db.ExecContext(ctx, `UPDATE workspace_mcp_install_requests
		SET status=?,decided_by=?,decided_at=?,decision_reason=?,installed_server_id=?
		WHERE workspace_id=? AND id=? AND status=?`, status, strings.TrimSpace(actor),
		time.Now().UTC(), strings.TrimSpace(reason), strings.TrimSpace(serverID),
		wsroot.Normalize(workspaceID), strings.TrimSpace(id), RequestPending)
	if err != nil {
		return err
	}
