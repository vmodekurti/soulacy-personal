...
func (s *Server) safeConfigView() fiber.Map {
	...
	return fiber.Map{
		...
		"auth": fiber.Map{
			"mode":               cfg.Auth.Mode,
			"jwt_access_ttl":     cfg.Auth.JWTAccessTTL,
			"jwt_refresh_ttl":    cfg.Auth.JWTRefreshTTL,
			"oidc_issuer":        cfg.Auth.OIDCIssuer,
			"oidc_audience":      cfg.Auth.OIDCAudience,
			"oidc_client_id":     cfg.Auth.OIDCClientID,
			"oidc_client_secret": oidcClientSecret,
			"oidc_redirect_url":  cfg.Auth.OIDCRedirectURL,
			"oidc_scopes":        cfg.Auth.OIDCScopes,
		},
		"runtime": fiber.Map{
...
type PatchableConfig struct {
	...
	// JWTSecret is deliberately excluded: first-run setup configures OIDC but
	// cannot replace the stable signing key generated by `sy setup`.
	Auth *struct {
		Mode             string   `json:"mode" yaml:"mode"`
		JWTAccessTTL     string   `json:"jwt_access_ttl" yaml:"jwt_access_ttl"`
		JWTRefreshTTL    string   `json:"jwt_refresh_ttl" yaml:"jwt_refresh_ttl"`
...
func (s *Server) handlePatchConfig(c *fiber.Ctx) error {
	...
	if patch.Auth != nil {
		candidate := *s.config()
		applyAuthConfigPatch(&candidate.Auth, patch.Auth)
		if err := candidate.Validate(); err != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "invalid authentication configuration: " + err.Error()})
		}
	}
...
func applyPatch(dst map[string]any, patch PatchableConfig) {
	...
	if patch.Auth != nil {
		auth := getOrCreateMap(dst, "auth")
		if patch.Auth.Mode != "" {
			auth["mode"] = patch.Auth.Mode
		}
		if patch.Auth.JWTAccessTTL != "" {
			auth["jwt_access_ttl"] = patch.Auth.JWTAccessTTL
		}
		if patch.Auth.JWTRefreshTTL != "" {
			auth["jwt_refresh_ttl"] = patch.Auth.JWTRefreshTTL
		}
		auth["oidc_issuer"] = strings.TrimSpace(patch.Auth.OIDCIssuer)
		auth["oidc_audience"] = strings.TrimSpace(patch.Auth.OIDCAudience)
		auth["oidc_client_id"] = strings.TrimSpace(patch.Auth.OIDCClientID)
		if patch.Auth.OIDCClientSecret != "" && patch.Auth.OIDCClientSecret != "***" {
			auth["oidc_client_secret"] = patch.Auth.OIDCClientSecret
		}
		auth["oidc_redirect_url"] = strings.TrimSpace(patch.Auth.OIDCRedirectURL)
		if patch.Auth.OIDCScopes != nil {
			auth["oidc_scopes"] = patch.Auth.OIDCScopes
		}
	}
	if patch.UI != nil {
...
func applyAuthConfigPatch(dst *config.AuthConfig, patch *struct {
	Mode             string   `json:"mode" yaml:"mode"`
	JWTAccessTTL     string   `json:"jwt_access_ttl" yaml:"jwt_access_ttl"`
	JWTRefreshTTL    string   `json:"jwt_refresh_ttl" yaml:"jwt_refresh_ttl"`
	OIDCIssuer       string   `json:"oidc_issuer" yaml:"oidc_issuer"`
	...
	if patch.JWTAccessTTL != "" {
		dst.JWTAccessTTL = patch.JWTAccessTTL
	}
	if patch.JWTRefreshTTL != "" {
		dst.JWTRefreshTTL = patch.JWTRefreshTTL
	}
	dst.OIDCIssuer = strings.TrimSpace(patch.OIDCIssuer)
	dst.OIDCAudience = strings.TrimSpace(patch.OIDCAudience)
	dst.OIDCClientID = strings.TrimSpace(patch.OIDCClientID)
	if patch.OIDCClientSecret != "" && patch.OIDCClientSecret != "***" {
		dst.OIDCClientSecret = patch.OIDCClientSecret
	}
	dst.OIDCRedirectURL = strings.TrimSpace(patch.OIDCRedirectURL)
	if patch.OIDCScopes != nil {
		dst.OIDCScopes = patch.OIDCScopes
	}
}
