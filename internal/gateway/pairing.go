...
func (s *Server) handleCreatePairingToken(c *fiber.Ctx) error {
	...
	pb, err := resolvePairBase(c.Context(), s.cfg.Server.PublicURL, c.BaseURL(), s.cfg.Server.Port, body.BaseURL, boundedProbe(pairProbeFor(s.tlsFingerprint), min(s.httpRequestTimeout(), pairProbeBudget)))
	if err != nil {
