...
//
// LocalAdaptive ties the pieces together: extraction (E27) produces
// candidates, resolution (E28) reconciles each one against the owner's active
// facts, and retrieval (E29) serves the prompt block. Storage is the SQLite
// fact store; embeddings come from whichever Embedder the host wires in (the
// same one Knowledge uses). Everything is scoped by FactScope.
package memory
...
func (l *LocalAdaptive) Remember(ctx context.Context, scope FactScope, turn Turn) (RememberOutcome, error) {
	...
	candidates, err := Extract(ctx, l.complete, turn)
	if err != nil {
		return out, err
	}
...
func (l *LocalAdaptive) Add(ctx context.Context, scope FactScope, category FactCategory, content string) (Fact, error) {
	scope = scope.Normalize()
	f := Fact{Workspace: scope.Workspace, Owner: scope.Owner, AgentID: scope.AgentID, Category: category, Content: content, Confidence: 1, Source: "manual"}
	f, err := prepareFact(f)
	if err != nil {
		return Fact{}, err
	}
...
func (l *LocalAdaptive) Update(ctx context.Context, scope FactScope, id, content string, category FactCategory) (Fact, error) {
	content, err := NormalizeFactContent(content)
	if err != nil {
		return Fact{}, err
	}
	return l.store.Update(ctx, scope, id, content, category, l.embed(ctx, content))
}

func (l *LocalAdaptive) Delete(ctx context.Context, scope FactScope, id string) error {
	return l.store.Delete(ctx, scope, id)
}
