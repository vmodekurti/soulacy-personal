...
var Resources = []Resource{
	...
	{Name: "knowledge", Class: WorkspaceOwned, ScopeKey: "workspace_id", Retention: "until KB/document deletion", Export: "source documents and manifest", Deletion: "purge document, chunks, embeddings, and ingest jobs", Backup: "workspace database and objects"},
	...
	{Name: "runs", Class: WorkspaceOwned, ScopeKey: "workspace_id", Retention: "run retention policy", Export: "run records and results", Deletion: "workspace delete", Backup: "workspace database"},
	...
	// A finished export is a complete copy of the workspace in one file,
	// downloadable with nothing but its opaque id — which makes it the
	// highest-value object this system produces, and the one most likely to
	// outlive the data it copied. Classified WorkspaceOwned rather than
	// Ephemeral despite its TTL: ephemeral would leave it out of the DELETION
	// plan, and an archive surviving a workspace deletion is exactly the
	// leftover full copy that matters most.
	// The limits a workspace has set on ITSELF (MU-030 criterion 1). Workspace
	// data rather than deployment configuration, because a workspace owner
...
var Tables = []Table{
	...
	{Source: "internal/memory/vector_reindex.go", Name: "memory_vector_reindex_stage", Resource: "vectors", Class: WorkspaceOwned, ScopeKey: "workspace_id,rowid", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/memory/vector_workspace_test.go"},
	{Source: "internal/memory/vector_reindex.go", Name: "memory_vector_reindex_state", Resource: "schema-metadata", Class: PlatformGlobal, ScopeKey: "none", Isolation: Scoped, IsolationTest: "internal/memory/vector_reindex_test.go"},
	...
	{Source: "internal/runcontrol/store.go", Name: "chat_run_controls", Resource: "sessions", Class: WorkspaceOwned, ScopeKey: "workspace_id,run_id", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/runcontrol/store_test.go"},
	...
	{Source: "internal/runtime/checkpoint.go", Name: "workflow_checkpoints", Resource: "sessions", Class: UserPrivate, ScopeKey: "workspace_id,agent_id,run_id,step_id", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/runtime/checkpoint_isolation_test.go"},
	...
	{Source: "internal/session/history_postgres.go", Name: "conversation_history", Resource: "messages", Class: UserPrivate, ScopeKey: "workspace_id,user_id", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/session/history_postgres_test.go"},
	...
	// Platform-global for the same reason as the version table: a schema
	// migration is a property of the installation, not of a tenant. The rows
	// name a component, a version and a file path — no workspace appears in
	// them, and the snapshot itself is a copy of a whole database file that
	// spans every tenant in it. Which is exactly why the endpoint that reads
	// it is gated to owners and admins rather than scoped per workspace.
	{Source: "internal/sqlitex/backupgate.go", Name: "soulacy_schema_backups", Resource: "schema-metadata", Class: PlatformGlobal, ScopeKey: "none", Isolation: Scoped, IsolationTest: "internal/sqlitex/backupgate_test.go"},
	...
	{Source: "internal/tenancy/postgres.go", Name: "credentials", Resource: "credentials", Class: UserPrivate, ScopeKey: "workspace_id via principal memberships", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/tenancy/postgres_test.go"},
	...
	{Source: "internal/tenancy/postgres.go", Name: "workspace_identity_providers", Resource: "credentials", Class: WorkspaceOwned, ScopeKey: "workspace_id (primary key),organization_id", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/tenancy/postgres_test.go"},
	{Source: "internal/tenancy/postgres.go", Name: "workspace_setup_tokens", Resource: "credentials", Class: WorkspaceOwned, ScopeKey: "workspace_id (primary key)", CompositeUniqueness: true, Isolation: Scoped, IsolationTest: "internal/tenancy/postgres_test.go"},
...
var Repositories = []Repository{
	...
	// redundantly, because a deleted workspace's records must be removable and
	// a hash cannot be searched by tenant.
	...
	// ServerStore name, and classifying it honestly is better than renaming
	// the type to slip past the scan.
	{Source: "internal/mcp/pool.go", Resource: "mcp", Class: Ephemeral, ScopeKey: "workspace_id (map key)", Isolation: Scoped, IsolationTest: "internal/mcp/tenantcreds_test.go"},
	{Source: "internal/workspaceexport/job.go", Resource: "workspace-exports", Class: WorkspaceOwned, ScopeKey: "workspace_id (directory namespace)", Isolation: Scoped, IsolationTest: "internal/workspaceexport/export_test.go"},
	...
	{Source: "internal/runcontrol/store.go", Resource: "sessions", Class: Ephemeral, ScopeKey: "workspace_id,run_id", Isolation: Scoped, IsolationTest: "internal/runcontrol/store_test.go"},
	...
	{Source: "internal/session/history_postgres.go", Resource: "messages", Class: UserPrivate, ScopeKey: "workspace_id,user_id", Isolation: Scoped, IsolationTest: "internal/session/history_postgres_test.go"},
	...
	{Source: "internal/tenancy/workspace_identity.go", Resource: "credentials", Class: WorkspaceOwned, ScopeKey: "workspace_id", Isolation: Scoped, IsolationTest: "internal/tenancy/postgres_test.go"},
