...
// Fork copies one workspace's conversation into a new session inside the same
// workspace. Every statement carries the tenant predicate: without it, a fork
// naming another tenant's session would copy that conversation into a session
// this caller can then read at will — a leak dressed up as a feature.
func (s *SQLiteHistoryStore) Fork(ctx context.Context, workspaceID, srcSessionID, newSessionID string, uptoEntryID int64) (int, error) {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM conversation_history WHERE workspace_id = ? AND session_id = ?`, workspaceID, newSessionID).
	...
	rows, err := tx.QueryContext(ctx,
		`SELECT subject, agent_id, role, content, tokens FROM conversation_history
		 WHERE workspace_id = ? AND session_id = ? AND id <= ?
		 ORDER BY id ASC`, workspaceID, srcSessionID, uptoEntryID)
	if err != nil {
		return 0, fmt.Errorf("session/history: fork select: %w", err)
	}
	type row struct {
		subject, agentID, role, content string
		tokens                          int
	}
	var src []row
	for rows.Next() {
		var r row
		if err := rows.Scan(&r.subject, &r.agentID, &r.role, &r.content, &r.tokens); err != nil {
			rows.Close()
	...
	// The copy keeps each turn's original subject rather than re-owning it to
	// the forker: a fork is a branch of the same conversation, and rewriting
	// the owner would quietly move someone else's turns into another person's
	// private history.
	...
	for i, r := range src {
		...
		if _, err := stmt.ExecContext(ctx, workspaceID, r.subject, newSessionID, r.agentID, r.role, r.content, r.tokens, createdAt); err != nil {
